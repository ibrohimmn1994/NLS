...
ESDIRK_ERK;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
iteration_M = sparse(I-k*a_s(2,2)*0.5*1i*D2);
[l,u,p,q,d] = lu(iteration_M);
d = sparse((1./diag(d)).*I);
pd = p*d;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% preconditioning using the approximate diagonal block matrix 
M = iteration_M;
M_ = zeros(size(M));
len = length(M_);
num = len/5;
for i = 1:num
    ii = i-1
    five = 5;
    M_(1+ii*five:five+ii*five,1+ii*five:five+ii*five) = ...
        M(1+ii*five:five+ii*five,1+ii*five:five+ii*five);
end
...
while t<t_1 
    ...
    for i=2:6
       ...
       % W_guess = iteration_M*(P*(V  + k*F_s +k*G_ns)+BC);
        W_guess = q* (u\ (l\ (pd* (P*(V  + k*F_s +k*G_ns)+BC))) );
   
       
        W_guess = P*(W_guess)+BC;
        K(:,i) =  0.5*1i*D2*W_guess; 
        K_hat(:,i) = - beta* 1i*( (abs(W_guess).^2).*W_guess );
...
%{
 if Control == "Yes"
   ...
   iteration_M =  (I-k*a_s(2,2)*D2*P)\I ;
 else
     V_new = P*(V + k*F_s_total + k*G_ns_total)+BC ;
     V = V_new;
     %disp("here")
